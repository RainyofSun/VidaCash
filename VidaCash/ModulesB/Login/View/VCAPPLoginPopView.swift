...
class VCAPPLoginPopView: VCAPPBasePopView {
    ...
    private lazy var phoneTextFiled: VCAPPNoActionTextFiled = {
        ...
        let view = VCAPPNoActionTextFiled.buildVidasdCashNormalTextFiled(placeHolder: tempStr)
        return view
    }()
    ...
    private lazy var voiceBtn: UIButton = {
        ...
        view.setAttributedTitle(NSAttributedString.attachmentImage("login_voice_code", afterText: false, imagePosition: -3, attributeString: VCAPPLanguageTool.localAPPLanguage("login_pop_voice_code"), textColor: RED_COLOR_F21915, textFont: UIFont.systemFont(ofSize: 15)), for: UIControl.State.normal)
        return view
    }()
    
    private lazy var loginBtn: VCAPPLoadingButton = VCAPPLoadingButton.buildVidaCashGradientLoadingButton(VCAPPLanguageTool.localAPPLanguage("login_pop_title"), cornerRadius: 25)
    ...
    private lazy var protocolLab: UIButton = {
        ...
        attributeStr.append(NSAttributedString(string: VCAPPLanguageTool.localAPPLanguage("login_pop_protocol2"), attributes: [.foregroundColor: RED_COLOR_F21915, .font: UIFont.systemFont(ofSize: 13), .underlineStyle: NSUnderlineStyle.single.rawValue, .underlineColor: RED_COLOR_F21915]))
        view.setAttributedTitle(attributeStr, for: UIControl.State.normal)
        return view
    }()
    
    private lazy var agreeBtn: UIButton = {
        let view = UIButton.buildVidaNormalImageButton("login_protocol_nor", selectedImg: "login_protocol_sel")
    ...
    override func buildPopViews() {
        ...
        if VCAPPCommonInfo.shared.isAppAudit {
            self.topImgView.image = UIImage(named: "pop_top_image_A")
        }
