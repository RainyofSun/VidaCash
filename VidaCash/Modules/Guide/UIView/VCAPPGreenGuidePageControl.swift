//
//  VCAPPGreenGuidePageControl.swift
//  VidaCash
//
...
enum VCPageControlAliment {
    case Center
    case Left
    case Right
}

protocol FVPageControlDelegate {
    func vc_pageControlClick(pageControl: VCAPPGreenGuidePageControl, index: Int)
}

class VCAPPGreenGuidePageControl: UIControl {
    ...
    /// 位置
    var pageAliment: VCPageControlAliment = .Center {
        didSet{
            if numberOfPages > 0 {
                createPointView()
            }
    ...
    /// 协议代理
    var delegate:FVPageControlDelegate?
    ...
    @objc private func clickAction(_ recognizer: UITapGestureRecognizer) {
        ...
        if let tag = recognizer.view?.tag {
            ...
            if index >= 0 {
                currentPage = index
                delegate?.vc_pageControlClick(pageControl: self, index: index)
