...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        // 开启网络监测
        VCAPPNetworkObserver.shared.StartNetworkStatusListener()
        // 初始化语言
        VCAPPLanguageTool.setAPPLocalLanguage(VCAPPDiskCache.readAPPLanguageFormDiskCache())
        // 设备认证
        VCAPPAuthorizationTool.authorization()
        // 初始化log 日志
        VCAPPCocoaLog.shared.registe(with: EnvType.other)
        // 读取本地化登录信息
...
private extension SceneDelegate {
    func setApplicationRootWindow() {
        ...
        let guideVC = VCAPPGreenGuideViewController()
        guideVC.guideDelegate = self
        self.window?.rootViewController = guideVC
        self.window?.makeKeyAndVisible()
    }
    
    func deviceAuthorization() {
        VCAPPAuthorizationTool.authorization().requestDeviceIDFAAuthrization { (auth: Bool) in
...
extension SceneDelegate: GreenGuideProtocol {
